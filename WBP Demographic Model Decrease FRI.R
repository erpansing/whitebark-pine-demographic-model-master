...
fire_current_year <- function(t, size = 1){
  ifelse(rexp(rate = lambda(t), n = size) > runif(n = 1, min = 0, max = 1), TRUE, FALSE)
}
...
project <- function(projection_time, n0, reps = 100, fire = T){       # stochastic projection function 
  ...
  for(j in 1:reps){       # Iterate through i years (projection_time) of population growth j times (iterations)
    ...
    #   intervals = NULL
    # }
    iteration <- append(iteration, rep(j, projection_time * reps), 
    ...
    for(i in 1:projection_time){           # get population
      ...
      fire <- ifelse(fire_current_year(t) > runif(1, 0, 1), TRUE, FALSE)  # Determine whether this iteration experiences a stand replacing fire
  ...
  results <- list(pop_sizes = pop_sizes,  LAI_track = LAI_tracker) #fire_intervals = fire_intervals,
